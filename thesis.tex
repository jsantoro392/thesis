...
%	data from each detector is time-shifted so that there is a temporal coincidence between the GW and the GRB in each IFO \cite{92}.
	...
	All simulated signals are modelled using the {\ttfamily SEOBNRv3} waveform approximate \fpg{Citazione}, which differs from the {\ttfamily IMRPhenomD} waveform model used to generate the templates: this is intentional and factors in our incomplete knowledge about waveforms.
	The injection parameters are chosen as follows \cite{208}.
        \begin{itemize}
...
\label{sec:pycbc00}
	The {\texttt{PyCBC}} coincident search targets GWs from CBCs with a total mass of $2$--$500\,M_\odot$,
        ...
        This analysis match filters each single detector data stream against a template bank (Sec.\,\ref{subsec:mfcbc}),
