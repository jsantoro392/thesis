...
\label{subsec:fittingfactor}
	...
	one can define the {\textit signal recovery fraction} of the injected signals against a template bank, as
	\begin{equation}
		\alpha = {{\int d \vec{x} \phi^3(\vec{x} \sigma^3(\vec{x}))} \over {\int d \vec{x} \sigma^3(\vec{x})}}
	\end{equation}
	where $\vec{x}$ is the source’s parameter vector (excluding the luminosity distance) 
	and $\sigma(\vec{x})$ is the horizon distance\footnote{The horizon distance is defined as the farthest luminosity distance of an optimally inclined (face-on) and oriented (overhead) CBC source at a fixed canonical SNR, normally taken to be 8.} 
	and is proportional to the signal power \cite{125}. 
	Including the $\sigma$ factors means that signals that 
	...
	It is also convenient to express the bank performance in terms of the {\textit effective fitting factor}, defined as 
	\begin{equation}
	FF_{eff} = \alpha^{1/3}
	\end{equation}
...
\subsubsection{Coherent False Alarm Probability}
	...
	of transient signals around the time of the GRB, and an {\textit on-source} window, 
	which is searched for GW signals. 
 	The on-source window is assumed to start five seconds before and ending one second after the GRB trigger time.
	
	To evaluate the p-value of the loudest event, with the loudest template,
	in this 6s window, the off-source data is analysed.
	The latter consists in 90 minutes of data around the on-source, 
	...
	into as many 6s segments as possible, in order to match the width of the on-source window.
	The data is then reanalysed, identifying the the loudest trigger in each off-source-trial \cite{46}.
...
%\end{table}
	...
	The effective fitting factor, calculated as explained in Sec.\,\ref{subsec:fittingfactor}, for the entire bank is 99\% for both sets, meaning a $\sim 6\%$ loss of detections: this is smaller than the 10\% goal we had set, so the bank passes the validation, or effectiveness, test.  \fpg{Lo 0.98 come lo hai ricavato? A quale set si riferisce? BNS o NSBH?  E qual \`e il valore per l'altro set?}
