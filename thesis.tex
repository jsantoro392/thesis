...
\label{subsec:coherent}
	...
	to effectively construct a single, more sensitive detector.
 	While this method is applied to searches for unmodelled burst sources,
        it is computationally expensive in the case of modelled searches \cite{45}.  It is therefore used for externally triggered searches, when knowing the time- and sky-location of the source can cut the cost of the search.  \fpg{Citare i vari papers metodologici di {\ttfamily PyGRB}.}
...
\subsection{GW150914: the Birth of Gravitational-Wave Astronomy}
	...
	The arrival time at Hanford relative to Livingstone was $\Delta t_{HL} = 7.0^{+0.02}_{-0.02}$s \fpg{Le unit\`a di misura!!!}.
...
\subsection{GW170814: the First Three-Detector Network Observation}	
	...
	pure the tensor polarizations of general relativity, over pure scalar or pure vector polarizations.
        ...
        \begin{figure}[!t]
          ...
          \centering
          \caption{Same as Fig.\,\ref{fig:o1} but for the O2 events: GW170104, GW170608 and GW170814.\fpg{Quali? Perch\'e 170104 appare due volte.}}
          \label{fig:o2}
        \end{figure}
...
\subsection{GW190521}
	The BBH that produced the GW event GW190521 was the most massive and energetic BH merger yet,
	with unprecedented and unexpected masses of $85^{+21}_{-14} M_{\odot}$ and  $66^{+17}_{-18} M_{\odot}$.
	The fact that the primary BH mass is $85 M_{\odot}$ is highly unusual, 
	since this is in conflicts with current models of stellar evolution:
	based on the theoretical understanding of the internal workings of massive stars, 
	and how BH form, it predicts that we should expect to find BH
	with either less than 65 solar masses or more than $120 M_{\odot}$, but none in between.
	The kinds of stars that would give rise to BH in this gap would not follow the usual “stellar mass” BH route,
	going supernova, and thus would not end up as BH. 
	...
	meaning the two progenitor BHs were themselves each the result of a previous merger before they found each other and merged.
	This multiple merger scenario requires that BHs form in special environments 
	where there are enough other BH nearby for multiple merger events to occur,
	such as dense clusters of stars or the disks of active galactic nuclei. 
	Another groundbreaking discovery that came from this event was the proof that intermediate-mass BH do exist,
	since the post-merger remnant BH mass is $142^{+28}_{-16} M_\odot$, 
	falling right into the mass gap between stellar-mass and supermassive BH.
	This event was detected with a three-detector network signal-to-noise ratio of 14.7, 
	and it is also the farthest detection so far with a luminosity distance of $5.3^{+2.4}_{-2.6}$Gpc,
	corresponding to an epoch when the universe was only about half its present age $z \sim 0.8$.

\subsection{GW190814}
	This was the most exciting yet puzzling observation so far, since the initial source classification,
	went back and forth between being a BBH and a NSBH.
	First of all, this is the most asymmetric system observed to date:
	the primary component is a BH with mass $23.2^{+1.1}_{-1.0}$, 
	while the the mass measured for the lighter compact object lies in the hypothesized 		
	“lower mass gap”, $m_2=2.59^{+0.08}_{-0.09}$ makes it either the lightest BH
	or the heaviest NS ever discovered in binary.
	However, it is comparable in mass to the BH produced by the merger of the two NSs observed in GW170817.
	The mass gap originates from observations of X-ray binaries, 
	where there seem to be no BH below about $\sim 5 M_\odot$,
	also supported by theoretical predictions \cite{195}.
	The hypothesis that the unknown object is indeed a BH, 
	requires an explanation for the lack of X-ray observation, 
	and for its production mechanism.
	On the other hand, a safe assumption for the NS upper limit mass is around 3 $M_\odot$:
	the range for observed pulsar masses is $2.1-2.7 M_\odot$ \cite{196},
	GW estimates have been made after GW170817 that the maximum NS mass should be below 2.2 or 2.3 $M_\odot$ \cite{197,198},
	SGRBs observations constraint the NS remnant mass to be less than 2.4 $M_\odot$.
	The scenario in which the secondary object is indeed a NS gives a new insight about the properties of NS matter, 
	since it requires rather stiff EOS with a maximum speed of sound $c_s \geq \sqrt{0.6}$ times the speed of light \cite{199}.
	There are two possible way to asses whether or not a NS was present in the binary: 
	a measurement of the effects of the tidal distortion of the NS 
	in the GW signal, and an electromagnetic counterpart.
	However, since tidal effects are weaker for more unequal mass binaries no such measurement was possible in this case. 
	Since it was a three-detector detection, the source localization was tight: about 240 Mpc away, 
	six times the distance of GW170817, meaning any light emitted would be about 36 times fainter.
	Even if this event was a NSBH merger, which under the right condition can be accompanied by an EM counterpart
	(as it will be described in Sec.$\ref{sec:emnsbh}$), it may be too faint to see, 
	and it was indeed missing from EM follow-up campaigns.
...
\fpg{Giri di correzione: 0.}%
	...
	In this chapter we will illustrate how to increase the search speed and sensitivity to EM bright GW events,
	...
	{\ttfamily PyGRB} is a matched filter search, therefore it is, foremost, that the template bank covers the region 
	of the parameter space of BNS and NSBH systems, 
	which are the GW sources for which electromagnetic follow-up is most promising.
...
\subsection{Evolution of Black Hole-Neutron Star mergers}
...
\subsection{Tidal disruption model}
	Tidal disruption occurs when  the  tidal force of the BH at the surface of the NS is stronger
        ...
        Whether or not this event can result in short GRBs emission,
...
\subsection{Electromagnetic Counterpart of a Black Hole-Neutron Star Binary Merger}
\label{sec:emnsbh}	
	There are two kinds of matter that characterise NSBH mergers and that are essential to generate an EM counterpart:
        ...
        which may lead to the production of a short GRB.
        As seen in Section $\ref{subsec:grb}$, CBC was a potential candidate for short GRB progenitors,
        and was indeed confirmed with the first EM-GW joint detection of a merging NS binary,
        as seen in Section $\ref{subsec:GW170817}$, cementing the link between short GRBs and BNS binary mergers.
...
\label{ch:datanalysis}
\fpg{Giri di correzione: 0.}%
	In a targeted GW follow-up of short GRBs search, a convenient way to increase its speed and sensitivity to EM bright GW events, 
	is to discriminate between regions in the parameter space that enclose combinations of masses and spins, 
	consistent with the minimum counterpart possibility requirement to have a NS in the binary, i.e. BNS and NSBH.
	For the latter systems we need to exclude the EM dim case of the BH swallowing the NS,
	and since GRBs may be ignited when a sufficiently massive accretion disk forms around the remnant BH, 
	an estimation of the remnant matter post merger is a useful indicator of whether a coalescence event is accompanied by an EM signal.         
	In the O2 coherent GW search triggered by short GRBs, the template bank was generated implementing the fitting formula $\ref{eq:mrem}$,
