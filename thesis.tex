...
\subsection{GRB 190627A}
	...
	the parameters of which are listed in Sec.\,\ref{sec:pygrb}, that serve as a simulated foreground. 
...
\fpg{Rifare sulla falsa riga della precedente sezione.  Per le capitons puoi mettere ``Same as Fig... but for GRB 190728271.''  Nel testo non dovrai rispiegare i plot.}
        ...
        Since the H1 single SNR plot  looks scattered, Fig.\,\ref{fig:detector2},
        with injections barely contained inside $3\sigma$,
        one could  decide to rerun the analysis excluding H1.
                \begin{figure}[h]
                        ...
                        \label{fig:detector2}
                \end{figure}
        By looking at the Found/Missed Injection plots in Fig.\,\ref{fig:missinj2},
        and comparing to other GRB analyses, it is noticeable that the injection are distributed closer
        than for most other runs, and there are also many nearby missed injections
        for both BNS($<68$Mpc) and aligned spin NSBH($<115$Mpc) injection sets.
        For the BNS set, 32 injections have FAP$\neq0$:
        a  closer look at the missed injections table shows that
        most of them have at least one spin magnitude $>$0.05 that also causes precession,
        ...
        This may be due to the large fractional in-plane spin magnitudes($\sim$0.032 and $\sim$0.036),
        since the calculated match between template and injection shows a good agreement.
        This is not the case for two of the ten aligned spin NSBH missed injections:
        a comparison between the SEOBNRv3 injection with a PhenomD waveform with the same physical parameters as it returns matches of  $\sim$8 and  $\sim$9.
        One of  the two also has high Null SNR values, together
        Out of the ten aligned spin NSBH missed injections, three do not have a NS spin magnitude $>$ 0.05:
        one has high Null SNR value, while  the second one suffers from differences between the waveform models,
        since  when comparing the SEOBNRv3 injection with a PhenomD waveform with the same physical parameters,
        return a match of $\sim$0.9. The third one has both a high Null SNR value and a low match ($\sim$8) between the injection and waveform model.
                \begin{figure}[h]
                        \noindent
                        \label{missinj2}
        ...
        ven though all curves reach $\sim100\%$ efficiency at nearby distances,
