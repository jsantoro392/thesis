...
\fpg{Qui pure: cita abbondantemente il paper di O2 perch\'e non \`e cambiato nulla e cos\`i giustifichi le scelte.}
	...
	the threshold for a GRB to be short is set to $T_{90} − \Delta T_{90} > 4\,$s \fpg{No, ricontrolla!}. 
...
\subsection{GRB 190627A}
	...
	At the time of GRB 190627A, two GW detectors, H1 and L1, were operating and their responses in the direction of the GRB were 0.316 and 0.452, respectively.
	The panels in Fig.\,\ref{fig:signalconsistencyplots1} show the results of the $\chi^2$-tests applied to the offsource background data
        (blue crosses), and to the injections (red crosses) that serve as a simulated foreground \fpg{Assicurati di aver definito questo injeciton set da qualche parte e qui metti un puntatore a tale spiegazione}.  From top to bottom, the panels report the template bank veto test, the autocorrelation veto test, and the standard $\chi^2$-test, as a function of coherent SNR \fpg{Il plot deve quindi avere solo i tre pannelli che al momento sono a sinistra, mantenendo il titolo di ognuno (che appare a sinistra).  Assicurati che siano della stessa dimensione.  Inserire puntatore a dove hai spiegato i tre test.}.
	The contour lines correspond to values of constant reweighted SNR.  The black line, in particular, has a value of \fpg{Lo sai?} which is the threshold value discussed in \fpg{Nel discutere il setup della search, devi dichiarare questo valore e qui richiamarlo.} The shaded region is therefore vetoed: any triggers that fall within it will fail that specific $\chi^2$-test and be discarded.
	The effect of non-stationarities in the data is evident in the first two plots, that clearly show several background triggers with high coherent SNR \emph{and} high $\chi^2$ values.  When an injection overlaps with a glitch in the background, it may be dragged in the veto area and therefore be missed, as seen from the plots, particularly for a single missed injection with coherent SNR $\sim 12$.  This behaviour is not surprising and corresponds to situations in which a physical GW signal can overlap with a noise transient and thus not be confidently detected. \fpg{La frase precedente che ti ho scritto \`e importante.}
        
        \begin{figure}[!t]
          \noindent
          \label{signalconsistencyplots1}
          \makebox[\textwidth]{\includegraphics[scale=0.5]{signalconsistencyplots1}}%
          \centering
          \caption{\textbf{Signal consistency plots.} The panels show the $\chi^2$-test results for background data (blue crosses) and injected signals (red crosses) as a function of coherent SNR.   From top to bottom, the test reported are: the template bank test (``Bank Veto''), the autocorrelation test (``Auto Veto''), and the standard, frequency bin $\chi^2$-test (``Chi Square'').  \fpg{Manca una spigazione delle linee a reweighted-SNR costante.} \fpg{See Sec\,\ref{...} for details on the three $\chi^2$-tests.}}
          \label{fig:signalconsistencyplots1}
        \end{figure}

	Additional signal consistency tests are reported in Fig.\,\ref{fig:detector1}. \fpg{Bastano i due plots di single detector di sinistra, messi uno affianco all'altro.  La null-stat non viene applicata per 2 IFOs.} 
	Here, the single detector SNR is shown as a function of the coherent SNR, for the background data and injected signals (blue and red crosses, respectively).
	The expected trend for astrophysical signals is given by the middle, green line, whereas the other two green lines, the magenta lines, and the cyan lines represent $1\sigma$, $2\sigma$, and $3\sigma$ deviations, respectively.
	The behaviour of H1, which in this case is less sensitive than L1, is qualitatively worse but still acceptable.  The background data clearly depart from the expected behaviour of astrophysical signals.
        Finally, the black horizontal line and the shaded area beneath it mark a search threshold that is no logner used, so we will not discuss it.
        \begin{figure}[!t]
          \noindent
          \label{detector1}
          \makebox[\textwidth]{\includegraphics[scale=0.5]{detector1}}%
          \centering
	...
	The plots in Fig.\,\ref{fig:missinj1} are an example of the results one looks at to inspect the performance of the search in recover injections.
        These examples refer to the sets of precessing BNS (left) and non-precessing NSBH (right) injections and display the injeciton distance as a funciton of the 
	injection time.  Qualitatively, one expects the search to perform progressively worse as the distances of the injections increase.
	The distance at which the analysis begins to fail at recovering injections depends on the sensitivity range of the GW detectors used in the analysis, at the time of the GRB and in its directions.
	In both plots, blue crosses indicate injections that are found and were more significant than any background event; as expected, these are found mainly nearby.
	Black crosses indicate that no trigger was found for that specific injections: as expected these results dominate large distances.
	Red crosses indicate that a trigger is found in correspondence with an injection, but it was vetoed.  This kind of results is found in an intermediate region, where the injection is weak and may be over taken by noise.  If a cross of this kind appears for a nearby injection, instead, it generally is due to the presence of noise glitch.
	Finally, coloured circles indicate that the injection was found, and not vetoed, but was not louder than all the background triggers;  
	the circles are colour-codeded with the FAP of the recovered trigger.
	As discussed in the Sec.\,\ref{sec:pygrb}, the FAP is assigned to a trigger by calculating the probability that a trial of the same duration as the on-source window 
	might contain a backgroung event louder than the statistics of the trigger.
        One expects circles to dominate an intermediate transition regime from not confidently recovered injections (blue crosses) to not recovered injections (black crosses).  This transition distance is further out for more massive systems, i.e., NSBHs, as the GW amplitude scales with the (chirp) mass of the source.
	We note that missed injections can also occur within the network range.  Aside from the aforementioned case of glitches being present in the data, this can be also be due to discrepancies between 
	...
	and consider a maximum NS spin of $0.05$, whereas the injections sets can admit precession and NS spin magnitudes up to $0.4$.  In other words, the parameter space of the injections is larger than the on strictly targetted by the template bank.
	In the case of this GRB follow up, there are 7 injections at nearby distances ($<100\,$Mpc), within the BNS injection set, that warrant additional consideration: 
	...
	The aligned spin NSBH injection set shows a single missed injection that warrants follow up, at around \fpg{Qual \`e?  Indica il tempo cos\`i uno sa di che crocetta parli nel plot.  Credo sia la croce rossa a -500s.}.  This had an antialigned BH spin of $-0.65$. 
	...
	therefore the injection is missed due to differences between the approximant used for the template (IMRPhenomD) and for the injection (SEOBNRv3).  These are intentially chosen to be different to account for our incomplete knowledge of the \emph{exact} signal emitted by compact binaries.
        All in all, the pipeline performance in recovering the injection sets is clean.  \fpg{Perch\'e i precessing NSBHs non li tratti?}
        ...
        \begin{figure}[!t]
          ...
          \centering
          \caption{\textbf{Found/missed injections.} Plots of the injections distance against time. Each panel represents a different injections set: on the left BNS set with precessing spins, on the righ NSBH sets with aligned spins. \fpg{Qua devi ripetere la spiegazione dei simboli.}}
          \label{fig:missinj1}
        \end{figure}

	The distribution of FAPs for the background events is shown in Fig.\,\ref{fig:loudestoffsourcevent1} as a function of the detection statistic, i.e., the reweighted SNR (or ``BestNR'').
	This distribution is expected to be roughly flat up until a BestNR value of about $6$ --- as triggers are kept only if $\rho_{\rm coinc} > 6$ --- and to fall off linearly in this log plot from there onwards.  This smooth linear fall off is the behaviour of a Gaussian distribution, which is what the pipeline aims at obtaining with its detection statistic.  The fall off should end at a BestNR  of about $8$, 
	which is the threshold for detection candidates. 
	Uneaven fall offs, or tails in the distribution are potentially signs of failures in correctly handling glitches present in the background data. 
	The specific distribution in Fig.\,\ref{fig:loudestoffsourcevent1} is smooth but has a small tail at the higher end of the reweighted SNR.
	An inspection of the list of loudest offsource events revealed that this is simply a plotting artifact, as the loudest event has an upper limit on its FAP, rather than a specific assigned value.  This upper limit coincides, of course, with the FAP of the second loudest event and hence the plotting artifact.

       \fpg{Io chiuderei qui, dicendo che il background \`e trattato bene, le injections sono recuperate in maniera decente e che quindi si pu\`o guardare l'onsource.  Questo \`e stato fatto, ma non puoi mettere i risultati perch\'e, ecc; quello che succede \`e che o si trova un candidate signal, o, per FAP non sufficientemente bassa, si determina un lower limit sulla distanza come quella distanza entro la quale la pipeline recupera il 90\% delle injections.  Siccome hai vari sets di injections hai una $D_{90}$ per ogni set, ovvero per ogni potenziale classe di sorgenti. Non mi addentrerei nelle exclusion distances per gli offsource trials: altrimenti devi spiegare le varie curve, barre d'errore, ecc.}  

       \fpg{Sono qui: le 3 GRBs le correggo, indipendentemente dalla ristrutturazione di tutto ci\`o che precede.} 
	...
	The offtrial results for BNS and NSBH with aligned spin injections are shown in Fig.\,\ref{fig:offtrial},
	with  the detection efficiency and exclusion distance plots, respectively.
	These curves should be span the the fraction range smoothly, 	
	and the loudest event FAPs should be also be randomly distributed in that range. 
	This is clearly the  case for this GRB analysis, which does not present any problems.
	For each set, all injections are recovered within a distance of $83.88$Mpc for BNS and of $156.5$Mpc for NSBH.
                \begin{figure}[h]
                        \noindent
                        \label{offtrial}
                        \makebox[\textwidth]{\includegraphics[scale=0.5]{offtrial}}%
                        \centering
			 \caption{Injection recovery around the time of GRB 190627A, as a function of distance. The  BNS  injection set is represented in the left column, the NSBH with aligned spin set in the column on the right. Top panels shows the fraction found louder than the loudest background event, while bottom panels the fraction louder than the loudest foreground event. }
			\label{fig:offtrial}
                \end{figure}

\subsection{GRB 190728271}
\fpg{Rifare sulla falsa riga della precedente sezione.  Per le capitons puoi mettere ``Same as Fig... but for GRB 190728271.''  Nel testo non dovrai rispiegare i plot.}
	The Fermi Gamma-ray Burst Monitor (GBM) triggered and located GRB 190728A at 06:30:36 UT on 28 Jul 2019.
	The on-ground calculated location, using the Fermi GBM trigger data, is $RA = \ang{356.7}$, $Dec = \ang{5.4}$
	with a statistical uncertainty of 5.7 degrees, while the duration is $T_{90} = 0.832 \pm 0.905$s.
	All three detector were operating at  the time of the GRB with the following antenna patterns: 
	H1 with 0.271, L1 with 0.665, and V1 with 0.463.
	While the $\chi^2$ plot looks good, the bank and auto veto plots present several vetoed injections, as seen in Fig.\,\ref{fig:scplots2}.
		\begin{figure}[h]
                        \noindent
                        \label{scplots2}
                        \makebox[\textwidth]{\includegraphics[scale=0.5]{scplots2}}%
                        \centering
                        \caption{\textbf{Signal consistency plots.}}
                        \label{fig:scplots2}
                \end{figure}
	Further investigations  revealed that most of them are vetoed due to the presence of a highly spinning and precessing component in the binary.
	A few BNS injections had also a NS with one spin magnitude  $> 0.05$, other than  in-plane spin components.
	There are some injections in the null stat vetoed area, dragged by glitches.  
	Since the H1 single SNR plot  looks scattered, Fig.\,\ref{fig:detector2}, with injections barely contained inside 3 sigma,
	one could  decide to rerun the analysis excluding H1. 
                \begin{figure}[h]
                        \noindent
                        \label{detector2}
                        \makebox[\textwidth]{\includegraphics[scale=0.5]{detector2}}%
                        \centering
                        \caption{First row shows the null SNR against the coherent SNR. Bottom rows show the H1 and L1 SNR against the coherent SNR.}
                        \label{fig:detector2}
                \end{figure}
	In the Found/Missed Injection plots in Fig.\,\ref{fig:missinj2}, there are many nearby missed injections 
	for both BNS and aligned spin NSBH injection sets. 
	Investigating the BNS missed injections revealed that most of them have at least 
	one spin magnitude $> 0.05$ that also causes precession;
	except one, whose spin magnitudes are $\sim0.038$ and $\sim0.036$, 
	but also has a high $\chi^2$-test value, which is the main cause of the BestNR down-weighting. 
	This may be due to the large fractional in-plane spin magnitudes, 
	since the calculated match between template and injection shows a good agreement.
	This is not the case for some aligned spin NSBH missed injections: 
	besides the one that does not have spin component  $> 0.05$,
	a comparison between the SEOBNRv3 injection with a PhenomD waveform with the same physical parameters 
	shows that these may be suffering from differences between the waveform models.
	        \begin{figure}[h]
                        \noindent
                        \label{missinj2}
                        \makebox[\textwidth]{\includegraphics[scale=0.4]{missinj2}}%
                        \centering
                        \caption{\textbf{Found/missed injections.} Plots of the injections distance against time. Each panel represents a different injections set: on the left BNS set with precessing spins, on the righ NSBH sets with aligned spins.}
                        \label{fig:missinj2}
                \end{figure}
	In order to make a decision about the conditions under which rerunning  the analysis,
	the last step is looking at the off trial results which shows that injections with higher distances are required.
	Even though all curves reach $\sim100\%$ efficiency at nearby distances, 
	the detection efficiency plots are not entirely smooth at nearby distances, as shown in Fig.\,\ref{}.
                \begin{figure}[h]
                        \noindent
                        \label{offtrial2}
                        \makebox[\textwidth]{\includegraphics[scale=0.5]{offtrial2}}%
                        \centering
			\caption{Detection efficiency curves and exclusion distance plots of the first run.}
                        \label{fig:offtrial2}
                \end{figure}
	The analysis rerun with higher distances is shown in Fig.\,\ref{}.
	Even though there still are many missed injections, caused by either a component having spin magnitude $> 0.05$, 
	or high $\chi^2$ value, or further in-plane spin components causing precession, 
	the off trial results have improved.
	Higher exclusion distances and FAPs are shown in Fig.\,\ref{}: the plateau in the detection efficiency curves and exclusion distance curves is gone, 
	but they are not completely smooth and the BNS plots shows dips at closer distances.
	Smoother curves were successfully achieved by rerunning the analysing one more time with even further distances.
                \begin{figure}[h]
                        \noindent
                        \label{offtrial2rerun}
                        \makebox[\textwidth]{\includegraphics[scale=0.5]{offtrial2rerun}}%
                        \centering
                        \caption{Comparison between the detection efficiency curves and exclusion distance plots of the first and second rerun.}
                        \label{fig:offtrial2rerun}
                \end{figure}
	Follow up investigations about glitches in the IFOs time series showed four spikes with high SNR values :
        the one at SNR=37.02 corresponds to scattered light, while the ones at 10.75 and 15.81 both are 
	the result of a blip like glitch around trigger time.
                \begin{figure}[h]
                        \noindent
                        \label{glitch_grb}
                        \makebox[\textwidth]{\includegraphics[scale=0.3]{glitch_grb}}%
                        \centering
                        \caption{Re-weigted SNR vs GPS time.}
                        \label{fig:glitch_grb}
                \end{figure}

\subsection{GRB 190425089}
\fpg{Rifare sulla falsa riga della precedente sezione.  Per le capitons puoi mettere ``Same as Fig... but for GRB 190425089.''  Nel testo non dovrai rispiegare i plot.}
	...
	V1 was collecting data at the event time, but its antenna factor is low, 0.47, 
	...
	while it looks clean for L1 and H1, besides a single injection outside the $3 \sigma$ region,
	the V1 plot is not as clean as the others, as seen in Fig.\,\ref{fig:detector3}. 
	 	\begin{figure}[h]
                        \noindent
                        \label{detector3}
                        \makebox[\textwidth]{\includegraphics[scale=0.3]{detector3}}%
                        \centering
                        \caption{Single detector SNR vs coherent SNR plots.}
                        \label{fig:detector3}
                \end{figure}
	By rerunning the analysis, the detection efficiency curves and exclusion distance curves
	look smoother than in the 3 detector analysis, and they do reach  $\sim 100\%$ and $90\%$ exclusion distances are found,
	which they did not before, as shown in Fig.\,\ref{fig:offtrial3}  and Fig.\,\ref{fig:offtrial_plot3}.
	 	\begin{figure}[H]
                        \noindent
                        \label{offtrial3}
                        \makebox[\textwidth]{\includegraphics[scale=0.3]{offtrial3}}%
                        \centering
                        \caption{First row: detection efficiency curves for the H1L1V1 analysis. Bottom row: detection efficiency curves after excluding V1.}
                        \label{fig:offtrial3}
                \end{figure}
	 	\begin{figure}[h]
                        \noindent
                        \label{offtrial_plot3}
                        \makebox[\textwidth]{\includegraphics[scale=0.3]{offtrial_plot3}}%
                        \centering
                        \caption{First row: exclusion distance plots for the H1L1V1 analysis. Bottom row:  exclusion distance plotsafter excluding V1.}
                        \label{fig:offtrial_plot3}
                \end{figure}
	However, the curves for precessing injection sets are not entirely smooth at nearby distances; 
	the NSBHPRECESSSEOBNRV3 curves suffer the most from this.
	By rerunning the analysis with closer injection for the precessing NSBH, 
	detection efficiency curves and exclusion distance plots slightly improve, 
	besides some dips at nearby distances, see Fig.\ref{fig:offtrial_closer}.
	 	\begin{figure}[h]
                        \noindent
                        \label{offtrial_closer}
                        \makebox[\textwidth]{\includegraphics[scale=0.3]{offtrial_closer}}%
                        \centering
                        \caption{Detection efficiency curves and exclusion distance plots with closer injection for the precessing NSBH.}
                        \label{fig:offtrial_closer}
                \end{figure}
	In order to increase the number of injections within the IFOs horizon,
	a new cut for the single detector SNR is introduced: $\rho=0$.
	Implementing this cut, the H1L1V1 and H1L1 analysis are rerun, both with closer injections.
	 	\begin{figure}[H]
                        \noindent
                        \label{offtrial_closer}
                        \makebox[\textwidth]{\includegraphics[scale=0.3]{offtrial_closer}}%
                        ...
                        \label{fig:offtrial_closer}
                \end{figure}
	In Fig.\,\ref{fig:offtrial_de_cut} and Fig.\,\ref{fig:offtrial_ed_cut} it is shown a comparison between the detection efficiency curves and 
	exclusion distance plots of the 3 IFOs and 2 IFOs analysis with the SNR cut and closer injections.
	In the first case they look a little smoother but still do not reach  $\sim 100\%$,
	while in the second case  there are higher exclusion distances and FAPs for all results, and the overall plots look smoother. 
	 	\begin{figure}[h]
                        \noindent
                        \label{offtrial_de_cut}
                        \makebox[\textwidth]{\includegraphics[scale=0.3]{offtrial_de_cut}}%
                        \centering
                        \caption{Comparison between the detection efficiency curves of the analysis with the SNR cut and closer injection for both the H1L1V1 (top row) and H1L1 run (bottom row).}
                        \label{fig:offtrial_de_cut}
                \end{figure}
	 	\begin{figure}[h]
                        \noindent
                        \label{offtrial_ed_cut}
                        \makebox[\textwidth]{\includegraphics[scale=0.3]{offtrial_ed_cut}}%
                        \centering
                        \caption{Comparison between the exclusion distance plots of the analysis with the SNR cut and closer injection for both the H1L1V1 and H1L1 run.}
                        \label{fig:offtrial_ed_cut}
                \end{figure}
...
\fpg{E qui dovrai spiegare cosa \`e un chunk, che hai usato solo due IFO, che i dati non erano con la calibrazione finale.}
        ...
        This analysis performs direct matched filtering of the data against a template bank,
        to calculate the SNR for each combination of detector, template waveform, and coalescence time as described in Sec.\,\ref{}.

\fpg{Qua \`e un po' tutto da scrivere.  Per le subsections puoi ricalcare i titoli delle sezioni della result webpage che intendi commentare.}
...
% \subsection{Chunk 29}
	...
	during which the coincident time was of 6.235 days (72\%) and the detector sensitivity is shown on the right panel of Fig.\,\ref{fig:range_time}.
                \begin{figure}[h]
                        \noindent
                        \label{range_time}
                        \makebox[\textwidth]{\includegraphics[scale=0.3]{range_time}}%
                        \centering
	...
	Going through the list of the loudest non-coincident detector triggers in both detectors,
	divided by template-length bins, no potential GW events were found.
	Triggers were mostly caused by close to loud gated glitches and blips. 
	...
	The same goes for L1: there  are no potential GW events and mostly scattered light. 
                \begin{figure}[h]
                        \noindent
                        \label{glitch}
                        \makebox[\textwidth]{\includegraphics[scale=0.3]{glitch}}%
                        \centering
                        \caption{Glitches in the omega scan for H1 on the left, and L1 on the right.}
                        \label{fig:glitch}
                \end{figure}
	Looking at the missed injections with SNR$>8$ in the various injection categories,
	showed that for BNS and NSBH sets they were all very quiet, namely SNR$<8$.
	Same for the BBH missed injections, except for a loud missed injection due to gated data in L1 at the time of merging, 
	seen in Fig.\,\ref{fig:missed_inj}. %seen in Fig.\,\ref{fig:missed_inj}.                 
		\begin{figure}[h]
                        \noindent
                        \label{missed_inj}
                        \makebox[\textwidth]{\includegraphics[scale=0.3]{missed_inj}}%
                        \centering
