...
\chapter{Black hole-neutron star binaries}
	...
	BHs and NS are the densest object in the universe resulting from gravitational collapse, 
	and a useful tool to probe gravity in the strong-field regime.
	Gravitational proof of their interaction would join the last kind of CBC source to be detected, 
	since we have already collected GW signal from BBH and BNS.
	This GW observation would not only be the last missing puzzle piece in GW astronomy,
	but it would also represent a crucial cornerstone in stellar evolution and dynamics, supradense nuclear matter, 	
	complex microphysics, and powerful EM phenomena due to the delayed 
	matter outflows at different timescales and frequencies,
	which can be insightful on the origin of SGRBs. 
	In fact, NSBH binaries are considered to be likely sources for the central engine of SGRBs, 
	as long as massive hot accretion disks are formed around the black hole.

	The study of NSBH mergers offers an opportunity to observe a general relativistic system in the strong field regime, 
	in the presence of material at supernuclear density, magnetic fields, shocks, and intense neutrino radiation.
	...
	Large number of BHs and NSs and the presence of heavy BHs can impact significantly the probability for NSBH binary to form and, possibly, merge. 
	After their formation, the two objects go through an inspiral phase, 
	during which the binary separation gradually decreases due to GW emission.
	Since in this phase the BH and NS radii are much smaller than the orbital separation, and the GW emission period is longer than the orbital one, 
	the two compact objects are well approximated by two point masses in an adiabatic orbit.
	While this part of the binary evolution is well understood within the post-Newtonian approximation \cite{195},
	throughout the late inspiral to the merger phase, the orbital evolution depends on tidal deformations effects 
	and the subsequent modification of the binary component’s interaction. 	
	In particular, in the merger phase and resulting remnant-formation phase, 
	the dynamics of the system depends strongly on the BH spin and the structure of the NS: 
	the radius and density profile, or its equation of state.
	Therefore in the orbits immediately preceding the merger, as during the merger itself, 
	these binaries and its non-linear physics can only be properly modelled by numerical simulations within a general-relativistic framework

	The detection of mixed BH-NS mergers is interesting for two reasons: 
	a key question is whether BH-NS and NS-BH binaries may be distinguished from one another 
	(i.e. the formation order of the two compact objects, which leads to a (mildly) recycled pulsar in the latter case), 	
	and the detected in-spiral of mixed BHNS mergers may reveal interesting deviations from GR and pure quadrupole radiation.

	Modelling the tidal disruption is of  great interest because GWs at tidal disruption would bring information about the NS radius 
	since the tidal disruption limit depends sensitively on it \cite{}. 
	The relation between the mass and the radius of NS may be used for determining the EOS of high density matter \cite{}. 


	% If the source of GW190425 is a BNS system, it is significantly different from the known population of Galactic double NS systems, with a total mass larger than any of the Galactic systems (Farrow et al. 2019; Zhang et al. 2019). This may have implications for the system’s origin, suggesting isolated formation in ultra-tight orbits with sub-hour orbital periods, or formation through the dynamical channel. Since we cannot see evidence of tides, it is possible that one or both objects could be BHs. However, this would require a previously unaccounted-for formation channel for binary BHs in this mass range.

\section{Evolution of Black Hole-Neutron Star mergers}

	The evolution of the NSBH system is expected to follow the same three-stage coalescence process as BNS or double BH:
	the secular inspiral due to GW emission, the actual merger and, 
	finally, the ring-down phase, settling into a final state, consisting of a BH.
	However, numerical simulations of the late phases indicate that mergers of NSBH binaries show two potential qualitative behaviours:
	the NS is tidally disrupted by the BH’s tidal forces or plunges into the BH, 
	depending on the mass ratio, spins, and the NS Equation-of-State (EoS). 
	The final fate of a NS depends primarily on the mass ratio, spins and NS EoS, as well as its compactness:
	when the BH mass is small enough or the NS radius is large enough, 
	the NS will be tidally disrupted before it is swallowed by the BH. 

	
	 \textbf{First case} If the NS is torn apart, material from the star is then quickly accreted onto the BH within a few milliseconds.
	Part of the remaining material outside the BH at $\sim 10 ms$ after merger is composed by baryon mass, 
	and is defined as the remnant mass $M_rem$, and usually find its equilibrium configuration in an accretion disk around the BH.
	These disks are usually fairly thick, have temperatures of a few MeV,
	can contain a significant fraction of the initial mass of the star (a few tenths of a solar mass).
	Nuclear reactions in the debris disk and ejecta, neutrino winds, and relativistic outflows 
	are examples of processes that power EM transients, therefore offer promising conditions for the generation of SGRBs.
	

	 \textbf{Second case}The second scenario lets the star reach the ISCO before it overflows its Roche lobe. 
	No tidal tail is formed, and the black hole and neutron star merge directly. 

	Numerical simulations have confirmed that disk formation is favored by a high black hole spin 
	(as the ISCO is closer to the black hole), a large neutron star (the star is more easily disrupted), 
	and a small black hole mass (the size of the Roche lobe at the ISCO relative to the radius of 
	the neutron star is smaller for lower mass black holes, thus making Roche-lobe overflow easier). 
	All existing simulations of BHNS megers in general relativity have studied black holes 
	of mass between 2 and 5 times the mass of the neutron star. 
	For such configurations, disk formation is the most likely scenario. 	
	It can only be prevented for anti-aligned spins \cite{}, and compact stars 	
	or massive nonspinning black holes \cite{}. 
	Mass shedding of a non-spinning NS occurs when the tidal force of 
	its companion BH at the surface of the NS is stronger than the self-gravity of the NS. 
	Population synthesis models tend favor distributions in which a large fraction of BHNS binaries 
	have a more massive black hole $M_{BH} > 10 M_{sun}$, especially for binaries formed in low-metallicity environments \cite{}. 
	These predictions come with large uncertainties, but it still appears important to study the behavior of BHNS systems for higher mass black holes.

	For the merger, one needs to include thermal effects, magnetic fields, nuclear reactions, neutrinos, etc.
	for an accurate description, particularly to compute multimessenger signals.
	The post-merger evolution of BNSs and NSBHs is highly affected by neutrino emission,
	particularly for the ejecta, which affect EM counterparts.
	No numerical relativity simulation to date attempts to model the neutron star crust as a solid, 
	and this is likely a good approximation for GW emission in most cases, 
	since the correction to the quadrupolar tidal Love number due to the crust’s elasticity is at most %\sim 1\%$ \cite{Biswas et al., arXiv 2019}. 
	However, crust cracking could lead to pre-merger EM signals \cite{Penner et al., ApJL (2012); Tsang et al., PRL (2012)} 
	and possibly even dissipative effects that could have a secular effect on the inspiral phasing \cite{Kochanek, ApJ (1992); Penner et al., ApJL (2012)}.
	While there are simulations including thermal effects, magnetic fields, and neutrinos, these are often included fairly simply, 
	and there is much more physics one might want to include, including nuclear reactions and even elasticity.
	


	%this is to be cited%
	Critically, Mrem impacts the observables of the plethora of possible EM counterparts, 
	and their detectability by current EM facilities. 
	For instance, Mrem, as introduced by FF12, is currently used when triggering EM follow-up searches 
	by alerts sent by the LIGO and Virgo detectors (see the method outlined in [26]). 
	Another application is GW measurements: tidal disruption (i.e. Mrem 6= 0) leads to a distinct shutoff 
	in the signal [27] that depends on the longsought after EoS of NS matter. 
	Previous work on modelling NSBH mergers has focusedon mass ratios Q = MBH/MNS  3, 	
	with MBH and MNS the gravitational masses of the compact objects in isolation (see [28–32] and references therein). 
	This range corresponds to astrophysical formation scenarios through supernova explosions in a progenitor 
	binary that predict a gap between NS and BH masses [33, 34]. 
	Comparable-mass binaries with a single NS could involve a primordial BH [35], 
	a BH born in a prior NSNS merger that formed a binary through dynamical interactions 
	in a dense cluster or galactic core (see the review in [36]), 
	or an exotic BH-like object (see [36, 37] for possible BH mimickers).
	In the inspiral phase, both the frequency and 
	the amplitude of the waves will increase, the system is said to “chirp”. 
	The orbit decays secularly until the last stable orbit, ISCO, is reached, 
	where the binary enters a “plunging phase”. 
	In this phase the stars fall nearly radially towards each other and 
	merge within about one orbital period. 
...
\chapter{Black hole-neutron star binaries}
	...
	They are the most simple fundamental objects that in the universe 
	...
	Both NS and BH represent the end states of the lives of massive stars 
	and their masses essentially range from one to a few solar masses, 
	crammed into objects with radii of ten to fifteen kilometers, and they are extraordinary cosmic laboratories.
	From the GW detection of these objects’ merger, we are able to extract 
	and learn about the sources fundamental properties, such as the BH’s masses and spins 
	as well as the NS equation of state (EOS); from the electromagnetic radiation we can not only 
	...
	in the presence of material at super-nuclear density, magnetic fields, shocks, and intense neutrino radiation.
	This is the optimal scenario to validate whether NSBH binaries are suitable SGRBs progenitor canditate.

	During their merger, NSBH binaries may also emit EM radiation isotropically, 
	as opposed to beamed SGRB emission, when they eject unbound material. 
	This can be as massive as $\sim 0.1M_{odot}$ and have subrelativistic velocities of $\sim 0.2-0.3c$ [12], 
	producing EM counterparts in the form of macronovae/kilonovae, powered by decay heat of unstable 
	r-process elements and by non-thermal radiation from electrons accelerated 
	at blast waves between the merger ejecta and the interstellar medium [13–18]. 

	Binaries containing at least one NS are particularly interesting because their GW signal contains 
	signatures of the physical conditions of matter at nuclear densities (see, e.g. [1– 9]) 
	and may thus eventually reveal the equation of state (EOS) of NSs, which is currently highly uncertain [10]. 
...
\section{Evolution of Black Hole-Neutron Star mergers}
	...
	depend crucially on whether or not the NS is tidally disrupted: there are three possible outcomes of the merger, 
	two of them involve the tidal disruption of the NS and one does not.
	Most of the GW emission occurs prior to the NS tidal disruption, 
	if this happens at all, and before significant thermal effects take place. 

	Further, magnetic fields appear to barely affect the GW signal [59]. 
	These circumstances imply that an ideal fluid-dynamics treatment with a cold EOS 
	and ideal-gas $\Gamma$-law for the thermal part are appropriate to simulate 
	the dynamical regime of interest for GW signals. 
	A notable feature of these signals is that the cutoff frequency at which 
	their amplitude damps due to the NS tidal disruption depends on the NS EOS. 
	Hence, the cutoff frequency encodes information on the EOS itself, particularly when this is stiff [19–22]. 
	This cutoff frequency is especially important in the construction of NSBH template banks. 
	If a BBH-like template built to detect a disruptive NSBH coalescence were to be truncated at 
	Targeting NSBH binaries with BBH templates terminated at a frequency that is too low 
	with respect to the physical cutoff frequency of the source, results in a loss in recovered SNR would occur. 
	On the contrary using an increased truncation frequency,
	it could become too high with respect to the physical cutoff frequency of the NSBH source, 
	and this would possibly result in penalizing the template by degrading its performance in chi-square tests, which would also be detrimental to the detection. 

	During the inspiral phase the BH and NS radii are much smaller than the orbital separation, 
	and the GW emission period is longer than the orbital one, 
	therefore the two compact objects are well approximated by two point masses in an adiabatic orbit

	While this part of the binary evolution is well understood within the post-Newtonian approximation \cite{195},
	throughout the late inspiral to the merger phase, the orbital evolution depends on tidal deformations effects 
	and the subsequent modification of the binary component’s interaction, 
	PN expansions may not converge rapidly enough in the strong-field region.
	In particular, in the merger phase and resulting remnant-formation phase, 
	the dynamics of the system depends strongly on the BH spin and the structure of the NS: 
	the radius and density profile, or its equation of state.
	Therefore in the orbits immediately preceding the merger, as during the merger itself, 
	...
	While numerical-relativity simulations are necessary to simulate this process accurately, 
	in the case of compact binaries containing at least one NS, modelling the NSBH waveform is puzzling, 
	both because long and accurate simulations are computationally expensive and because the parameter space, 
	which include the relative mass ratio, the stellar compactness, and the black hole spin, is larger. 


\subsection{Tidal disruption model}

	Simulations indicated that the NS tidal disruption is a characteristic feature 
	of the dynamics of quasi-circular BHNS mergers. 
	On the contrary, quasi-circular binary NS mergers with mass ratio $q < \sim 2$ 
	do not present significant tidal disruption, e.g., [19, 20]. 
	Physically, tidal disruption is expected if the binary reaches a characteristic radius $rTD$ 
	before the innermost stable circular orbit (ISCO), the radius of which we denote by $rISCO$. 
	$rTD$ is expected to scale in the same way as the mass-shedding radius $rMS$, 
	which is determined by the condition that the BH tidal force overcomes the NS self-gravity 
	at the stellar surface $RNS: rTD < \sim rMS$, with a weak dependency on the BH spin [21]. 
	Because $RNS/MBH = (qC)−1$, where $C = MNS/RNS$ is the NS compactness, the ratio that regulates the onset of 
	tidal disruption is $\eta = rTD/rISCO$. Thus, tidal disruption depends on three physical parameters: the binary mass ratio, the BH spin and the NS compactness. 
