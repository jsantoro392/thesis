...
\subsection{Missed Injections}
        Figure \ref{fig:missed_inj} summarizes the outcome of the injection campaigns carried out on the chunk of O3b data.  \fpg{Si fanno $N$ injections, con  waveform model... e prior sui parametri... (forse sono gi\`a scritti da qualche parte nella tesi).} In both panels, red crosses represent injections that were not recovered, while circles are recovered injections coloured by their FAR, the dark blue ones being identified undoubtedly as signals, that is, they were found to be louder than all background events.  In both panels, the chirp mass of the source may be read off the horizontal axis.  The vertical axis in the left panel is the decisive SNR, that is, the expected SNR in the second most sensitive instrument to that particular source, while in the panel on the right hand side it is the source distance.  As the chirp mass of the signal increases, the recovery is possible up to progressively higher distances, as the signal amplitude is directly proportional to the chirp mass, and inversely proportional to the distance.
         ...
         \begin{figure}[t]
          ...
          \centering
          \caption{{\bf Injeciton results.} {\textit Left:} Each point represents the source of a simulated GW signal injected in the data and recovered from it.  The horziontal axis reports the chirp mass of the source, while the veritcal one is the decisive optimal SNR of the signal, which is the expected SNR in the second most sensitive instrument to that particular source.}
          \label{fig:inj_plots}
        \end{figure}
