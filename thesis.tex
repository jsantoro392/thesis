...
\fpg{Giri di correzione: 0.}%
	More than 100 years ago, Albert Einstein predicted the existence of gravitational waves (GWs) on the basis of his theory of general relativity \cite{1,2}.
	He discovered that the motion of masses
	can cause distortions in the curvature of spacetime that 
        travel throughout spacetime itself spreading out at the speed of light, similarly to the ripples in a pond when a stone is tossed in it.
	...
	Nowadays, however, we are able to directly detect GWs.  The most energetic GWs are generated when high speeds and compact ojects are involved, that is, celestial bodies with a high concentration of gravitational mass per unit volume.
	Orbiting pairs of black holes (BHs) and neutron stars (NS), the densest object in the universe,
	are therefore efficient GW emitters and are indeed present in all the GW sources detected by the LIGO-Virgo Collaboration so far.
        ...
        they have a singularity in them, no surface, and an event horizon separating the information
        ...
        On the other hand, NSs are extreme objects from the point of view of all fundamental forces, as their masses fall in the range $1$--$2.5M_\odot$, while their radii are of the order of 10\,km; additionally, they are equipped with magnetic fields that can exceed $10^{15}\,$G (in this case one refers to the NS as magnetar). 
	...
	In the final fraction of a CBC, a significant amount of gravitational mass is converted into gravitational energy.  This travels outwards as GWs and these waves fall within the frequency range of current terrestrial GW detector, if the binary contains NSs and/or BHs with masses up to $\mathcal{O}(100M_\odot)$.

        \fpg{Qui devi dire che LIGO-Virgo cercano rountinely binarie compatte (che vengono in 3 sapori: BNS, NSBH, BBH).  Sia in bassa latenza che offline e con varie pipelines. Indeed in questa tesi, tu hai condotto, fra le altre cose, una ricerca di segnali CBC con la pipeline PyCBC.  (Sii breve e metti forward references alle sezioni con i risultati e alle spiegazioni su PyCBC).}
	The GW network of LIGO-Virgo is consistently looking for signal from 
	compact binary mergers of NSs and BHs (BNS, NSBH, and BBH systems). 
	Search pipelines are run both online, designed to detect and report compact binary merger events 
	with sub-minute latencies, and offline, taking advantage of improved calibration of the data 	
	and additional information regarding data quality.
	On the other hand, the Burst searches are capable of detecting signals from a wide variety of astrophysical sources 
	in addition to compact binary mergers: core-collapse supernovae, seismic activity in NSs, and yet unknown GW sources.
	
	However, the main focus of this work is the CBC matched filtering search with the PyCBC pipeline discussed in Chapter $\ref{ch:datatools}$, 
	which I used to conduct the offline data analysis for the LIGO-Virgo third observing run (O3), presented in Chapter $\ref{ch:datanalysis}$.

        The masses of the objects involved dictate the how long the GW signal they emit lasts in the band of current detectors.
        More massive binaries merge at lower frequencies and therefore have fewer cycles in the sensitivity band.
        ...
        Binary NS (BNS) systems, in contrast, accumulate many cycles in the instrumental band.
        The first BNS coalescence witnessed by LIGO-Virgo, GW170817, generated a GW signal that lasted 100 seconds in the sensitivity window of the detectors \cite{15}.
	...
	The observation of GW150914 was a truly remarkable accomplishment: it \emph{directly}\footnote{Until then, GWs had only been observed indirectly by using pulsars timing to monitor 
        their effect on the orbit of pulsar binary star systems.} confirmed Einstein's prediction of the existence of GWs, and it demonstrated that stellar-mass binary BH (BBH) systems form in the universe and merge within a Hubble time.
	...
	which enables the observations of violent astrophysical events that are not visible with traditional astronomy.
        ...
        \fpg{Qua invece riprendi GW170817.  Scrivi che fra le altre cose conferma che le BNS prodcuono short GRBs.  Poi ingloba (riducendolo/adattandolo) il tuo materiale GRB che trovi qui sotto.  Devi arrivare a giustificare la necessit\`a di fare ricerce coerenti in corrispondenza di GRB {\bf e} l'utilizzo dell'EM bright bank.  Queste due cose sono centrali nella tesi e devi motivarle proprio nell'introduzione.  Poi puoi dire che tu hai fatto anche questo tipo di ricerca e hai enabled la ricerca costruendo e validando la template bank (e di nuovo forward references).}
	
	The neutron star merger GW170817 marked the beginning of multi-messenger astronomy 
	because it was the first gravitational wave event with a recorded electromagnetic counterpart.
	As the NSs merged, they released a short flash of gamma rays, 
	which was independently detected by the Fermi Gamma-ray Burst Monitor, 
	less than 2s after the GW event was detected by LIGO and Virgo.

	Gamma-Ray Bursts (GRBs) are short ($\Delta t \sim 0.1-100s$) and intense bursts of 
	electromagnetic radiation in the high energy (1 keV–10 MeV) $\gamma$-ray spectrum, 
	that occur at the end of certain stars’s life.
	This powerful radiation is produced when an ultra-relativistic fireball of high energy photons, 
	is launched from some central engine, and is usually accompanied by a detectable afterglow emission 
	in the X-ray and sometimes optical and radio bands, as the brilliant fireball expand and cool.
	They can be broadly classified based on the timescale in which from 5\% to 95\% of the counts are recorded (defined as $T_{90}$),
	as either long gamma ray burst (LGRBs), $T_{90} > 2s$ or short gamma ray bursts (SGRBs), $T_{90} \geq 2s$.
	They also show different spectral properties: SGRBs have a fainter afterglow and are less energetic than LGRBs;
	SGRB light curves vary on very short timescales, which indicates a compact source smaller than a stellar-scale object, 
	and are also seen to have an offset relative to their host galaxy center.
	This is suited to a progenitor involving old astrophysical objects which has a significant velocity with respect to its host galaxy,
	exactly the expectation for compact binary mergers.

	The event GW170817 marked the first direct observation of a BNS as the central engines that power SGRBs,
	and not only confirmed a hypothesis that was as-yet unconfirmed about the origins of SGRBs,
	but it also allowed the measurement of the rate of expansion of the universe and 
	the comparison between the speed of gravity and the speed of light. 
	Furthermore, subsequent multi-messenger observations showed that these events 
	help explain the origin of the universe's heavy elements like gold. 
	There remain many mysteries surrounding these extreme and energetic events, 
	and multi messenger astronomy has proven to be a resourceful way to obtain 
	a complete knowledge of the astrophysical sources and their emission engines, 
	providing insight that electromagnetic observations alone cannot achieve:
	understanding the physics of jet breakout through measurements of time-delay 
	between the prompt gamma-ray emission and merger, investigate the SGRBs jet opening angles, 
	and provide an independent measurement of distance and redshift for use as a probe of cosmology.
	In addition SGRBs represent an appealing trigger for GW follow-up searches: 
	the time and sky position of the GRB are used to analyze the GW data around 
	the GRB time and considering GWs incident from its sky position.
	In this way only a small fraction of the parameter space of an all-sky GW survey needs to be searched,
	therefore it is possible to significantly reduce the detection threshold for the targeted coherent SGRB search,
	thereby increasing its sensitivity. 
	Since the observed properties of SGRBs may be attributed to matter accretion onto a BH,
	following the tidal disruption of a NS, neutron star-black (NSBH) hole systems are also considered likely SGRBs progenitors.
	In the time of writing this work, no NSBH coalescence both in the GW and electromagnetic (EM) spectrum 
	has been detected. 
	Therefore, it is extremely important to have updated data analysis tools to improve the performance of 
	offline GW searches following SGRB triggers, as the one I conducted in Chapter $\ref{ch:datanalysis}$ for three GRB samples.
	The targeted coherent search for the third observing run benefitted from the implementation of the 
	EM bright bank, which was generated after I implemented in the template bank the upgraded model $\ref{eq:mreup}$, 
	that better discriminates between disrupting and non-disrupting NSBH systems.
	I also demonstrate the efficacy of the EM bright bank, injecting known GW signals and determining how well the bank recovers them. 

	There remain much more to understand about CBC and SGRBs, 
	and multi messenger astronomy has proven to be a resourceful way to obtain 
	a complete knowledge of the astrophysical sources and their emission engines, 
	providing insight that electromagnetic observations alone cannot achieve.
	This should motivate future efforts towards targeted, coherent GW search triggered by SGRBs.

\fpg{Infine metti il tuo piano della tesi.  Nel capitolo 1..., nel 2..., ecc.}
	This thesis is organized as follows: Chapter I provides a theoretical introduction to gravitational waves and 
	describes the LIGO and Virgo detectors and the physics behind its purpose, and noise sources.
	Chapter II gives an introduction to data analysis tools for a matched filtering source, 
	...
	Chapter III gives a summary of the gravitational wave detection from the first to the third observing run.
	Chapter IV illustrates the difference between the old and new model for the remnant mass of a neutron star-black hole system.
	It then describes how the template bank has been updated and how well it works.
	Finally, the last sections present the result of the offline PyCBC and PyGRB search,
	and a brief discussion on the obtained results is provided in Chapter V.
%----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
\section{Template Banks}
	...
	The normalised overlap between the two waveforms is given by
		\begin{align}
			(\hat{h}_i|\hat{h}_j)& = {{(h_i|h_j)} \over {\sqrt{(h_i| h_i)(h_j| h_j)}}} \\
                			     & = (h_i|h_j).	
		\end{align}
	As usually the template waveforms are defined up to an overall normalization, 
	in the second line the waveforms are normalized to $(h|h) = 1$. \\
	...
	and to a down-weighting by the signal-based vetoes, see Sec. $\ref{subsec:chi_square_test}$ \fpg{(see Sec.\,...)}. \\
...
\subsection{Coincident analysis}
	...
	this window is taken as the time of flight for GW between the sites plus a small, fixed amount to allow for timing errors. 
	...
	10ms maximum travel time between detectors and 5ms padding to account for timing errors.
        This methodology ignores the polarization phase information at the expense of some loss in sensitivity,
	...
	The PyCBC pipeline, used in Sec. $\ref{sec:pycbc00}$, 
	...
	Assuming that the times of transient noise artifacts present in the data streams are not correlated between different detectors 
	and that GW signals are sparse in the data set,
	the significance of a GW event is a measure of the probability that it is a false detection due to coincident noise. 
\subsection{Coherent}
...
\subsection{GW170104: a 50-Solar-Mass Binary Black Hole Coalescence}
	...
	analyses is dwarfed by the signal rate, yielding a probility of astrophysical origin greater than $1 - (3 \times 10^{-5})$. \fpg{Citazioni.}
...
\subsection{GW170608}
	...
	PyCBC and GstLAL then identified GW170608 with a network SNR of 13,
...
\subsection{GW170814: the First Three-Detector Network Observation}	
	...
	pure the tensor polarizations of general relativity, over pure scalar or pure vector polarizations.
        ...
        \begin{figure}[!t]
          ...
          \centering
          \caption{Same as Fig.\,\ref{fig:o1} but for the O2 events: \fpg{Quali? Perch\'e 170104 appare due volte.}}
          \label{fig:o2}
        \end{figure}
...
\subsection{GW170817: the Birth of Multimessenger Astronomy}
	...
	The observation with excellent localization of GW170817 triggered a hugh electromagnetic follow-up campaign \fpg{Cita il 170817 multimessenger paper} which produced observations across the whole electromagnetic spectrum.
		...
		\begin{figure}[H]
			...
			\label{banktest}
                ...
                \begin{figure}[H]
                        \noindent
                        \label{fittingfactor}
                        \makebox[\textwidth]{\includegraphics[scale=0.4]{fittingfactor}}%
                        \centering
                        \caption{}
