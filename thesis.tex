...
\subsection{GRB 190627A}
	...
	whose parameters are listed in Sec.$\ref{sec:pygrb}$, that serve as a simulated foreground. 
	...
	which is the threshold value discussed in Sec.$\ref{sec:pygrb}$.
	...
	\fpg{La frase precedente che ti ho scritto \`e importante.}
	...
	at around \fpg{Qual \`e?  Indica il tempo cos\`i uno sa di che crocetta parli nel plot.  Credo sia la croce rossa a -500s.}.  
	...
	\fpg{Perch\'e i precessing NSBHs non li tratti? -Perchè i plot di NSBHprecessingseobnrv3 ce li ho tagliati a metà.}
        ...
        \begin{figure}[!t]
          ...
          \centering
          \caption{\textbf{Found/missed injections.} Plots of the injections distance against time. Each panel represents a different injections set: on the left BNS set with precessing spins, on the righ NSBH sets with aligned spins. Black cross indicates no trigger was found coincident with the injection. Red cross indicates a trigger was found coincident with the injection but it was vetoed. Coloured circle indicates that a trigger was found coincident with the injection but it was not louder than all offsource events. The colour bar gives the FAP of the trigger. }
          \label{fig:missinj1}
        \end{figure}
	...
	as injections are adequately recovered, the analysis can procede by looking at the on-source data.
	This step has been done, however, since the results are not made public we cannot discuss them.
	In order to claim detection of a GW candidate event, if present in the on-source data, 
	must have very low FAP ($\sim10^{-4}$);
	otherwise we can infer a lower limit on the 90\% exclusion distance, $D_{90}$, 
	defined as the distance within 90\% of the injections is recovered.
	The pipeline finds a $D_{90}$ for each injection set.
...
\fpg{Rifare sulla falsa riga della precedente sezione.  Per le capitons puoi mettere ``Same as Fig... but for GRB 190728271.''  Nel testo non dovrai rispiegare i plot.}
	...
	All three detector were operating at the time of the GRB 190728271, 
	and their responses in the direction of the GRB were: H1 with 0.271, L1 with 0.665, and V1 with 0.463.
	While the $\chi^2$ plot looks rather clean, one injection ends up in the bank veto grey area,
	while the auto veto vetoes several injections, as seen in Fig.\,\ref{fig:scplots2}.
		\begin{figure}[h]
                        \noindent
                        \label{scplots2}
                        ...
                        \label{fig:scplots2}
                \end{figure}
	Further investigations revealed that the injection vetoed by the Bank test was lost because it is a 22$M_\odot$,
	highly spinning and precessing BH with a precessing neutron star.
	Of the six vetoed injections for the Auto Veto, three of them are highly spinning and highly precessing BNS,
	one NSBH with highly spinning and precessing NS and highly precessing BH,
	while the last two are BNS with both spin magnitudes greater than 0.05 and in-plane spin components.
	Several injections overlapping with gilitchy data,
	end up in the NullStat veto area. 
	...
	one could  decide to rerun the analysis excluding H1. 
                \begin{figure}[h]
                        ...
                        \centering
                        \caption{Same as Fig.$\ref{fig:detector1}$ but for GRB 190728271.}
                        \label{fig:detector2}
                \end{figure}
