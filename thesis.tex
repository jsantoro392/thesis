...
%
\fpg{In vari punti usi {\ttfamily cite\{\}} oppure {\ttfamily ref\{\}}. Inoltre citi senza averle in biblio 213 e 215.  Nella bibliografia hai varie entries solo come arXiv preprints, anche per roba gi\`a pubblicata!}%
...
\subsection{GW170814: the First Three-Detector Network Observation}	
	...
	spanning hundreds to about a thousand square degrees at the 90\% credible level \cite{165, 166, 167}. 
	...
	polarisations \cite{171,172} can characterise metric perturbations.
...
\section{The Third Observing Run (O3)}
	...
	as well as a binary, whose smallest component is an unidentified massive compact object \cite{207}.
...
\label{subsec:GW1901412}
	...
	multipoles with $l = m$ gain increasing importance \cite{187, 188, 189, 190, 191, 192}.
...
\subsection{subsec:GW190814}
	...
	as it requires a rather stiff equation of state with a maximum speed of sound $c_s \geq \sqrt{0.6}c$ \cite{199}.
...
\label{sec:grbfollowup}
	The outstanding detection of GW170817 (Sec.\,\ref{subsec:GW170817}) 
        ...
        due to the probable compact binary nature of their progenitors \cite{162},
...
\label{subsec:o3aTemplateBank}
	...
	is to produce a sufficiently massive remnant accretion disk after the merger process \cite{}. % \fpg{}.
	...
	that targets only BNSs and NSBH systems that produce a remnant accretion disk after merger \fpg{Cita anche il paper O1, oltre alle due citazioni che seguono e che ho aggiunto.} \cite{136, 55}, as suggested in  \fpg{Cita Pannarale \& Ohme 2014}.  This kind of template bank is often referred to as ``EM-bright bank,'' as it targets only potentially electromagnatically bright compact binary sources.
        ...
        The first fit of this kind was reported in \cite{206}.
	A more recent, remnant mass fitting formula was published in \cite{205}:
...
%\label{subsec:mrem}
	...
	The accretion disk can spread to larger radii to conserve angular momentum in the rotational period $\sim 10\,$ms,
        ...
        Foucart (2012) \cite{206} proposed a model to calculate the amount of matter remaining
        ...
        The model for $M_{\rm rem}$ was upgraded by Foucart and collaborators (2018) \cite{205}, who proposed:
        \begin{equation}
          \label{eq:mremup}
...
\label{subsec:construction_and_validation}
	In light of the discussion so far, an EM-bright bank cover the portion of compact binary parameter space that
        ...
        \item Galactic BHs have masses that fall in the interval $[5,-15]M_{\odot}$ \cite{195},
...
%\end{table}
	...
	The O3a EM-bright template bank produced once the code was able to hande Eq.\,(\ref{eq:mremup}) is shown in Fig.\,\ref{fig:banktest}.  The two axes represent the mass of the BH and the magnitude of its spin, while the color code is the NS mass.  The only information lost in this plot is the NS spin magnitude, but this is taken to be 0.05 at most, so this direction in parameter space is not very rich.
        ...
        \fpg{Sono qui.}
%	The template bank is designed to have an SNR loss of no more than 3\% due to discretisation, which
        results in a bank of 288,177 templates. 	
	...
	combining the theoretical and experimental upper bound, the NS mass is restricted in the range $1M_\odot \leq M_{\rm NS}  \leq 2.8M_\odot$,
	while the dimensionless spin magnitude to $\leq 0.05$, which is based on NS parameters from known Galactic BNS systems \cite{155},
       	In a similar fashion, the parameters range for the BH is chosen to be $1M_\odot \leq M_{\rm BH}  \leq 25M_\odot$ and $0 \leq \chi_{\rm BH} \leq 0.998$.
...
\label{subsec:pygrb}
	...
	Further trigger selection get accomplished after calculating the $\rho_{\rm coinc}$, 
	and rejecting any trigger with $\rho_{\rm coinc}<6$, and then repeating the same procedure for the $\rho_{\rm coh}$.
	The last cut is done by computing the $\rho_{\rm null}$ and keeping only the triggers that satisfy the following conditions \cite{46}:
		\begin{equation}
			\rho_{\rm null} \leq 5.25 \hspace*{2cm} \text{and} \hspace*{2cm} \rho_{\rm coh} \leq 20
		\end{equation}
		\begin{equation}
			\rho_{\rm null} \leq {{\rho_{\rm coh}-20}\over {5}} \hspace*{2cm} \text{and} \hspace*{2cm} \rho_{\rm coh} > 20
		\end{equation}
	This prevents losing possibile loud GW events, that have high $\rho_{\rm null}$ and $\rho_{\rm coh}$ values,
	...
	The re-weighting is done in two steps: first according  to  the  $\chi^2$ value
		\begin{equation}
		\text{if} \hspace*{0.5cm} \chi^2 \leq n_{\rm dof} \hspace*{3.9cm} \rho_{\chi^2} =  \rho_{\rm coh} 
		\end{equation}
		\begin{equation}
		\text{if} \hspace*{0.5cm} \chi^2 > n_{\rm dof} \hspace*{1.2cm} \rho_{\chi^2} =  {{\rho_{\rm coh}}  \over {\{[1+ \big( {{\chi^2} \over {n_{\rm dof}}} \big)^3 ]/2\}^{1/6}}}
		\end{equation}	
	and then according to the null SNR
		\begin{equation}
			\text{if} \hspace*{0.5cm} \rho_{\rm null} \leq 4.25 \hspace*{3.5cm} \rho_{\rm rw} = \rho_{\chi^2}
		\end{equation}
		\begin{equation}
			\text{if} \hspace*{0.5cm} \rho_{\rm null} > 4.25 \hspace*{2cm} \rho_{\rm rw} = {{ \rho_{\chi^2}}  \over {\rho_{\rm null} - 3.25}}
		\end{equation}
	For each template of the mass parameter space, 
	the re-weighted SNR is calculated, and the template producing the largest $\rho_{\rm rw}$ 
	during the on-source window is retained as the event candidate \cite{46}. 
	For the {\ttfamily PyGRB} analysis, the threshold that a candidate event must surpass is $\rho_{\rm rw} \geq 8$,
	while, in order to remove any non-Gaussian triggers left from the data, 
	any trigger with $\rho_{\rm rw}$ less than 6 is discarded.
	The last step is to determine the significance of a trigger,
	namely how often glitches with a higher $\rho_{\rm rw}$ than the trigger will occur,
	...
	The injections parameters are chosen in the following way \cite{136}:
		\begin{itemize}
			\item $M_{\rm BH}$ are drawn from a Gaussian distribution with mean $10M_\odot$ and width $6M_\odot$.
			\item $\chi_{\rm BH}$ are drawn uniformly from the interval $[0.00, 0.98]$.
			\item $M_{\rm NS}$ are drawn from a Gaussian distributions with mean $1.4M_\odot$, 
	...
	even under the most optimistic assumptions regarding the NS equation of state and the amount of $M_{\rm rem}$ required to power the GRB emission \cite{}. 
...
\subsection{GRB 190627A}
	...
	139\,Mpc for precessing NSBHs, and 211 Mpc for aligned NSBHs, as reported in \cite{208}.
...
\subsection{GRB 190728271}
	...
	A high NS spin magnitude ($\sim 0.3$) also caused a single nearby NSBH injections to not be recovered as louder than all the background events.
        \begin{figure}[!t]
          ...
          \centering
            \includegraphics[width=0.67\textwidth]{GRB190728271_H1_snr_vs_snr_zoom}\\
            \includegraphics[width=0.67\textwidth]{GRB190728271_L1_snr_vs_snr_zoom}\\
            \includegraphics[width=0.67\textwidth]{GRB190728271_V1_snr_vs_snr_zoom}%
        ...
        \begin{figure}[!t]
          \noindent
          \label{missinj2_3}
          \makebox[\textwidth]{\includegraphics[scale=0.45]{missinj2_3}}%
          \centering
	...
	The loudest onsource event was given a FAP of 0.513, so it is not of astrophysical origin.  The $90\%$ confidence exclusion distances for the on-source window are 160\,Mpc, 204\,Mpc and 272\,Mpc for the BNS, the precessing and aligned NSBH, respectively, as reported in \cite{208}.
\begin{comment}
\begin{tabular}{ |p{3cm}||p{3cm}|p{3cm}|p{3cm}|  }
 \hline
 Source & $D_{90}$\\
...
\subsection{GRB 190425089}
	...
	The results of the $\chi^2$-tests are shown in Fig.\,\ref{fig:signalconsistencyplots3_final}:
        ...
        \begin{figure}[!t]
          ...
          \makebox[\textwidth]{\includegraphics[scale=0.68]{signalconsistencyplots3_final}}%
          \centering
	...
	follow the expected astrophysical trend overall, with just one injection falling out the 3$\sigma$ deviation in H1.  This corresponds to a BNS with highly spinning and precessing NSs and is therefore not surprising.
        \begin{figure}[!t]
          ...
          \makebox[\textwidth]{\includegraphics[scale=0.4]{detector3_final}}%
          \centering
          \caption{\textbf{Signal consistency plots.} Same as Fig.\,\ref{fig:detector1} but for GRB 190425089. These plot show the re-analysis of  GRB 190425089 without V1.}
        ...
        \begin{figure}[!t]
          \noindent
          \label{missinj3_5}
          \makebox[\textwidth]{\includegraphics[scale=0.44]{missinj3_5}}%
          \centering
	...
	and the on-source data results were investigated: 
	the loudest event had a FAP of 0.075 which is not significant astrophysically.  The 90\% confidence exclusion distance for BNS, generic spin NSBH and aligned spin NSBH sources were found to be 204\,Mpc, 247\,Mpc and 440\,Mpc, respectively, as reported in \cite{208}.
...
\fpg{E qui dovrai spiegare cosa \`e un chunk, che hai usato solo due IFO, che i dati non erano con la calibrazione finale.}
	...
	\fpg{S\`i ma in base a che vengono divisi i chunks?  Il run parte e si apre il chunk 1: come se ne sceglie la fine?}
	The boundaries of the chunks are chosen in the following way:
	the time at which the chunk starts in when at least one detector is observing;
	...
	The data immediately available, denoted by the string ``C00’’,  
	has higher uncertainty in the function used to convert from voltage to strain, 
	than later calibration versions that take longer to compute. 
	Therefore, this C00 data require the final data quality checks and calibration to be carried out.
	...
	%Chunks of data denoted by the string ``C00'' require the final data quality checks and calibration to be carried out.
        ...
        I analysed a chunk of C00 data running from 01:31:32 UTC on December 20, 2019 to 15:40:36 UTC on December 28, 2019.  
	I considered data from the two LIGO detectors.  
	This chunk contained 8.59 days of raw calibrated data,
...
\begin{thebibliography}{500}
	...
	\bibitem{207} The LIGO Scientific Collaboration, the Virgo Collaboration. GWTC-2: Compact Binary Coalescences Observed by LIGO and Virgo During the First Half of the Third Observing Run. arXiv:2010.14527.
	\bibitem{208} The LIGO Scientific Collaboration, the Virgo Collaboration. Search for Gravitational Waves Associated with Gamma-Ray Bursts Detected by Fermi and Swift During the LIGO-Virgo Run O3a. arXiv:2010.14550.
\end{thebibliography}
