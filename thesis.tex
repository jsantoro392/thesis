...
\label{sec:pygrb}
	...
	so that an estimate of the background noise in our detector network around the time of the SGRB can be estimated.
	The FAP of the on-source trigger is the fraction of off-source trials with an event of equal or greater significance than in the on-source. 
 	...
 	but it also allows to place constraints on the SGRB source in the event of no detection in the on-source window. 
	...
	The injections parameters are chosen in the following way:
		\begin{itemize}
			...
			\item In all three sets, the SGRB is considered to be emitted in the direction of the binary total angular momentum. 
	...
	in order not to neglect SGRBs in the tail of the duration distribution, 
	the threshold for a GRB to be short is set to $T_{90} − \Delta T_{90} > 4$s. 
	The GRBs analysed here were detected by Swift BAT or Fermi GBM.

\subsection{GRB190627A}
	GRB190627A  was a Swift detection of a burst that occurred at 11:18:31 UTC on June 27 2019,
	The BAT on-board calculated location is  $RA=\ang{244.83}$, $Dec = \ang{-5.3}$ with an uncertainty of 3 arcmin.	
	This was observed to have a duration of $T_{90} (15-350 keV) = 1.60 \pm 0.28$s, 
	which places it at the longer end of the SGRB distribution. 
	...
	For each set, all injections are recovered within a distance of $83.88$Mpc for BNS and of $156.5$Mpc for NSBH.
                \begin{figure}[h]
                        ...
                        \centering
			 \caption{Injection recovery around the time of GRB190627A, as a function of distance. The  BNS  injection set is represented in the left column, the NSBH with aligned spin set in the column on the right. Top panels shows the fraction found louder than the loudest background event, while bottom panels the fraction louder than the loudest foreground event. }
			\label{fig:offtrial}
                \end{figure}
...
\subsection{GRB190425089}
        GRB190425089 was observed at 02:07:43 on April 25, 2019 by the Swift satellites,
