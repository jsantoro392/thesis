...
\fpg{Giri di correzione: 1.}%
	...
	In the PyGRB analysis the classification is defined in a slightly different way: 
	given the error on the $T_{90}$, $|\delta T_{90}|$,
        when $T_{90}-|\delta T_{90}| > 4\,$s the GRBs are defined long,
	short when $T_{90}+|\delta T_{90}| \leq 2\,$s,
        while the rest are labelled as ambiguous.
...
\label{subsec:GW190425}
	...
	BHs with masses smaller than any BH mass observed so far \cite{175} \fpg{Citazione errata}.
	...
	louder than any background event up to its detection \cite{175}\fpg{Stessa cosa.  Ma l'articolo di GW190425?!}.
...
\subsection{Missed Injections}
 The injection and recovery campaign consists in generating signals with different waveforms model,
        with simulated source parameters generated by random draws from the prior;
        these sets of simulated GW signals are then injected in the detectors data streams.
        Next, the pipeline runs on such injection sets to estimate the search sensitivity  to the corresponding sources.

        Figure \ref{fig:inj_plots} summarizes the result of one of the injection campaigns carried out on the chunk of O3b data.
        Sets of simulated GW signals are injected in the detectors data streams

 \fpg{Si fanno $N$ injection sets che differiscono per waveform model e prior sui parametri; qui si mostrano a titolo d'esempio i risultati per una categoria che comprende... un po' tutto mi sa, ma controlla.  Le altre categorie mostrano comportamenti analoghi.}
	...
	since they have shorter templates that produce higher SNR triggers. 
        ...
        are the best parameters that encode the expected sensitivity of the search.
         ...
         \begin{figure}[t]
          \noindent
          \label{inj_plots}
          \makebox[\textwidth]{\includegraphics[width=\textwidth]{inj_plots}}%
          \centering
	...
	The missed injections in my chunk of data were lost mostly because of very low SNR values.
	However, the loudest missed injection is a BBH simulated signal shown in the top panel of Fig.\ref{fig:gated},
	with an decisive optimal SNR of $\sim 230$ and chirp mass of $\sim 40$,
	...
	The background investigations gave to overall good results,
	leading to further investigation on the foreground,
	which were shared with the analysis group within the collaboration.
