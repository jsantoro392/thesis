...
\subsection{Interaction of gravitational waves with test masses}
	...
	To understand how GWs interact with the interferometric detectors, that will be described in %warning: rephrase Section%  
...
%warning: rephrase se uno sa monitorare la distanza propria fra due test-masses, allora sa rilevare GWs.%
...
%warning: rephrase GW detectors need to be designed in order to maximise their sensitive to the part proportional to the Riemann tensor while minimizing their sensitivity to all other terms (see Section ...).%
%	In order to leave only the part proportional to the Riemann tensor, 
%	earth-based detectors look for higher frequency GWs, 
%	as GWs with low frequencies are comparable with slowly varying Newtonian noises. 
...
%warning:Queste 3 frasi vanno benissimo anche prima, quando mostri che serve la deviazione geodetica.  Ti permette poi di andare avanti fino all'eqazione precedente, senza mai dover dire rapidamente com'è fatto un IFO %
	...
	If the displacement between geodesics is $\zeta_a (t) = (x_0 + \delta x(t), y_0 + \delta y(t))$, then of $\zeta_a (t)$ is %warning: cosa avrò voluto dire?%
...
%warning:  modificare immagine phi è \omega t%
		\begin{figure}
...
\subsubsection{Compact Binary Spin}
	...
	have used templates that neglect the spins of the compact objects. \cite 
	...
	having been demonstrated that aligned-spin templates pull in more signal than noise. \cite{}
...
\subsection{$\chi^2$ test}
	...
	the matched-filter SNR is re-weighted 
			...
			\begin{equation}
				\hat \rho = \rho \hspace*{2cm}  for \chi^2_r <= 1
			\end{equation}
	
			\begin{equation}
				\hat \rho = \rho[{1 \over 2} (1 + (\chi^2_r)^3)]^{-1/6}  \hspace*{2cm}  for \chi^2_r > 1
			\end{equation}
...
\subsection{False Alarm Rate}
	...
	noise background events in the observation time with a detection-statistic value above that of the candidate event, given by 
		...
		\begin{equation}
			FAP(\hat \rho) \equiv P(\geq 1 noise event above \hat \rho|T,T_b) = 1 - e^{-T{{1+n_b(\hat \rho)} \over{T_b}}}
		\end{equation}
