...
\chapter{Gravitational wave observation so far}
	...
	is measure by the BNS range \cite{53, 54}.
	...
	The first GW direct observation was achieved in 2015 \cite{52}, 
	after LIGO began observing in an upgraded program called Advanced LIGO, 
	during its first observing run (O1) from 12th September 2015 to 19th January 2016.
	After an upgrade and commissioning period, 
	the second observing run (O2) took place from November 30, 2016 to August 25, 2017. \\
	During O1 and O2 the individual LIGO detectors were in science mode, meaning that they were taking data,
	for about 60$\%$ of the time, 45$\%$ of which was taken simultaneously by both detectors.
	...
	An example of reevaluation is one of the BBH mergers from O1, that was previously referred to as LVT151012.
 	Its statistical significance was to low to be considered as a GW at the time, 
	but a detailed re-analysis showed that it is an indeed a GW event, and it is now re-labeled as GW151012. 
	...
	The GW Transient Catalog (GWTC-1) \cite{58} from the O1 and O2 data collects 
	a total of eleven confident detections: 
	ten BBH mergers \cite{52, 57-60} and one BNS merger \cite{62} and fourteen marginal triggers.
	Notable events in this catalog are the  first observed event GW150914 \cite{52},
	the first three-detector event GW170814 \cite{61} and the BNS 
	coalescence GW170817 \cite{62}, the first case where an electromagnetic counterpart to a GW 
	has been observed from a single source \cite{63}. \\
	On April 1, 2019, Advanced LIGO and Advanced Virgo initiated their third observing run (O3),
	which ended in April, 2020 \cite{53}. 
\section{O1}
