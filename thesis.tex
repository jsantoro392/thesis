...
\section{Template Banks}
	...
	can be written as  
		\begin{equation}
			(h(f;\lambda)| h(f;\lambda + \Delta \lambda)) \equiv 4 \int_{0}^{\infty} df {{\tilde h(f;\lambda) \tilde h^*(f;\lambda + \Delta \lambda)} \over {S_n(f)}}.
                \end{equation}
	The normalised overlap between the two waveforms is given by
		\begin{equation}
			(\hat{h}(f;\lambda)|\hat{h}(f;\lambda + \Delta \lambda))= {{(h(f;\lambda)| h(f;\lambda + \Delta \lambda))} \over {\sqrt{(h(f;\lambda)| h(f;\lambda))(h(f;\lambda+ \Delta \lambda)| h(f;\lambda + \Delta \lambda))}}}
                \end{equation}
