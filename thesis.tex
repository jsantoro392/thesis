...
\subsection{GRB 190627A}
	...
	Follow-up electromagnetic observations of 190627A detected the optical counterpart of the GRB, 
	and by analysing the spectrum it was inferred that $z = 1.942$ is the likely redshift of GRB 190627A.
	At the time of the burst, two detectors were operating with the  following antenna patterns:
	H1 with 0.316, and L1 with 0.452.
                \begin{figure}[H]
                        \noindent
                        \label{scienceseggrb1}
                        \makebox[\textwidth]{\includegraphics[scale=0.5]{scienceseggrb1}}%
                        \centering
                        \caption{Science data at the time of a GRB from all available detectors is retrieved and then the software decides what data are to be analysed.
	...
	to ensure there is enough data to accurately estimate the power spectral density (PSD).}
                ...
                \end{figure}
	The signal consistency plots in Fig.\,\ref{fig:signalconsistencyplots1} contain the plots showing the $\chi^2$-test results for bakground
	in blue crosses, and injection in red crosses; the test are done both with and without injections, where only  background triggers are present.
	Different rows correspond to different $\chi^2$-tests. 
	The black line and dark shaded regions denote the cut level: any triggers in the shaded area will have been cut by the  $\chi^2$-tests. 
	The yellow contour lines show lines of constant re-weighted SNR. 
	The presence of a non-signal, non-Gaussian noise component in the data can drag an injection into the veto area, causing it to be missed.
	For this GRB, this happens to a single missed injection with coherent SNR $\sim 12$.
		\begin{figure}[H]
                        \noindent
