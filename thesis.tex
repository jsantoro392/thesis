...
\section{Injections}
	...
	Software injections are time series of simulated gravitational waveforms that are 
	added via software to detector data.  Adding them to data synthetically, rather than physically displacing the test masses to simulate a signal, has the advantage of not affecting the duty cycle of the detectors. 
        The waveforms are generated starting from a synthetic population of a certain type of source, 
	described as a collection of astrophysical parameter values.  These include the strain amplitude, sky location, 	
	and initial frequency \cite{30}. 
	The injections are separated in time by specifying a time-step to place them. 
	To randomize the placement, a time interval needs to be provided and the an injection is placed in this time interval around 
	each time-step.
	Further, one can choose to perform multiple sets of software injections, each one with its distinct set of parameters. 
	Thus, one can study the recovery of thousands of injections to decide on the best tuning for a search, and assess the performance of a search once the tunings are fix \cite{47}.
...
\subsection{GW170104: a 50-Solar-Mass Binary Black Hole Coalescence}
	...
	a mass-weighted combination of the spin components perpendicular to the orbital plane; the credible interval found was $\chi_{\rm eff} = −0.12^{+0.21}_{−0.30}$. 
...
\subsection{GW190412: a Binary-Black-Hole with Asymmetric Masses}
	...
	Even though the $\chi_{\rm eff}$ is found to be positive, large in-plane spin components are absent, 
...
\fpg{Giri di correzione: 0.}%
        In this chapter we illustrate how to increase the search speed and sensitivity to EM bright GW events of the {\ttfamily PyGRB} offline search \fpg{Non \`e assolutamente vero: non abbiamo toccato la pipeline migliorandone le prestazioni.},
        by implementing a new model \fpg{Se dici new model, ci vuole una citazione.} that helps discriminating between regions in the parameter space that enclose combinations of masses and spins,
        ...
        Therefore, an estimation of the remnant matter post merger is a useful indicator of whether a coalescence event is accompanied by an EM signal.  \fpg{questo \`e vero e questa frase andrebbe sfruttata meglio.}

	{\ttfamily PyGRB} is a matched filter search that focus on GW sources for which electromagnetic follow-up is most promising \fpg{No.  PyGRB prende uno specifico tipo di evento EM e fa il follow-up alla ricerca di GWs}, 
	...
	in the NSBH parameter space.
        ...
        \fpg{Ok, questo paragrafo introduttivo \`e interamente da rifare.  Tu hai: aggiornato la procedura per costruire EM bright banks, costruito la O3a bank, validato la O3a bank, usato PyGRB su 3 GRBs di O3a, usato PyCBC su un chunk di O3b.}
...
\subsection{GRB 190627A}
	...
	The effect of non-stationarities in the data is evident in the first two plots, that clearly show several background triggers with high coherent SNR \emph{and} high $\chi^2$ values.  When an injection overlaps with a glitch in the background, it may be dragged in the veto area and therefore be missed, as seen from the plots, particularly for a single missed injection with coherent SNR $\sim 12$.  This behaviour is not surprising and corresponds to situations in which a physical GW signal can overlap with a noise transient and thus not be confidently detected. \fpg{La frase precedente che ti ho scritto \`e importante.}
        ...
        \begin{figure}[H]
          ...
          \centering
          \caption{\textbf{Signal consistency plots.} Plots of the $\chi^2$-tests is that down-weight or cut loud background triggers that do not resemble the signals. First row shows the “Bank Veto”: it compares the power in the trigger with a small subset of template  waveforms from the template bank. Second row is  the “Autocorrelation Veto”:  it compares the power in the trigger with the veto time-shifted template. The last row contains the $\chi^2$-test that compares the power across frequency bins. }
          \label{fig:signalconsistencyplots1}
        \end{figure}


       \fpg{Sono qui: le 3 GRBs me le leggo, indipendentemente dalla ristrutturazione di tutto ci\`o che precede.} 

	When a detector sensitivity is lower compared to the other detectors, 	
	this can affect the efficiency of the search.
	The decision to exclude the data from the least sensitive detector, 	
	is made after looking at the single IFOs plots, in Fig.\,\ref{fig:detector1}, 
	that shows the single detector SNR cuts: 
	the expected trend for the jnjections is to be inside the error lines, wich corresponds to $3\sigma$;
	if they are scattered  outside these lines instead, it means that the IFO wasn't working properly at  the time of the trigger.  
	In  the latter case, excluding the less sensitive IFO from the analysis, leads to find better exclusion distances.
	For the two most sensitive detectors in the SNR network, 
	a lower threshold is placed on the SNR:
	if a trigger is not above threshold in any of the two most sensitive it is cut.
	It is expected that the injections for the Null SNR against the coherent SNR, 
	will place themselves at low values, while the background triggers should be found with high values. 
        In this case all the plots look very clean.
		\begin{figure}[H]
                        \noindent
                        \label{detector1}
                        ...
                        \label{fig:detector1}
                \end{figure}
	The injections plots in Fig.\,\ref{fig:missinj1} show the ability of the search to recover injections at different distances, 
	against time. It is expected that the analysis is able to better detect nearby injections than far ones. 
	The panel on the left represents the BNS injection set, while the one on the right the NSBH with aligned spin.
	Injections are expected to be found at a distance which is compatible with the IFOs range at the time of the GRB, 
	which can be computed combining the antenna patterns and the sensitivity range.
	Blue crosses indicate that the injection was found and was more significant than event in the background data. 
	Red crosses indicate that a trigger was found that was coincident with the injection, but it was vetoed. 
	Black crosses indicate that no trigger was found that was coincident with the trigger. 
	Coloured circles indicate that the injection was found but was not louder than all of the background, 
	and in this case the circle colour indicates the FAP of the trigger.
	More distant injections tend to be vetoed, missed completely, or have a relatively high FAP.
	However, there are also missed injections within  the IFOs network range and this can be caused by discrepancies between 
	...
	and consider a maximum NS spin of $0.05$. 
	Therefore BNS missed injections  might have dimensionless with a magnitude  greater than $0.05$,
	or either the NS or the BH could have spin components orthogonal to the orbital angular momentum,
	causing the precession to be dominant.
	Another reason behind a  missed injection could be a high $\chi^2$-value, caused by a template rang off by a glitch.
	For this GRB there are 7 missed injections at distance less than 90\% confidence exclusion distance for the BNS injection set.
	These all have precessing spins in both components and magnitude $>0.05$.
	The NSBH with aligned spin show one missed injection with $\chi_{BH} = -0.65$. 
	...
	therefore the missed injection is due to differences between the IMRPhenomD and SEOBNRv3 modules.
                \begin{figure}[H]
                        \noindent
                        \label{missinj1}
                        ...
                        \label{fig:missinj1}
                \end{figure}
	As already seen in the previous Sec.\,\ref{sec:pygrb},
	the FAP is the estimated probability that a trial of the same length as the on-source window 
	might contain a noise event louder than a specific statistic value. 
	The background significance as FAP is shown in Fig.\,\ref{fig:loudestoffsourcevent1} against the detection statistic.
	The BestNR curve should fall off at a BestNR value of $\sim6$, steeply until a value of $\sim8$, 
	which is  the candidate threshold. 
	If there is a tail in the distribution, this could be due to a failure to remove glitches. 
	In Fig.\,\ref{fig:loudestoffsourcevent1} the BestNR falloff looks clean but there is a small tail;
	however,  inspecting the loudest offsource event table doesn't reveal any problems.
                \begin{figure}[h]
                        \noindent
                        \label{loudestoffsourcevent1}
                        \makebox[\textwidth]{\includegraphics[scale=0.4]{loudestoffsourcevent1}}%
                        \centering
                        \caption{\textbf{Loudest offsource events.} Background significance as false alarm probability (FAP) against the detection statistic, BestNR.}
...
% \subsection{Chunk 29}
	...
	The same goes for L1: there  are no potential GW events and mostly scattered light. 
                \begin{figure}[h]
                        ...
                        \makebox[\textwidth]{\includegraphics[scale=0.3]{glitch}}%
                        \centering
                        \caption{Glitches in the omega scan for H1on the left, and L1 on the right.}
                        \label{fig:glitch}
                \end{figure}
